...
import "../style/loginInformer.scss"
import { useInformer } from '../hooks/useInformer'

const InformerLogin = () => {
  const navigate = useNavigate()
  const { loading, handleLoginInformer } = useInformer()
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      await handleLoginInformer(formData.email, formData.password)
      setFormData({ email: '', password: '' })
      navigate('/informer/dashboard')
    } catch (err) {
    ...
    <main>
      <div className='informerLogin'>
        ...
        <div className='loginbox3'>
          <p className='loginPtag'>New here? <Link to={"/informer/register"}>Create new Account</Link></p>
        </div>
